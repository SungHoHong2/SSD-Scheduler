...
// array size for Heap
#define STAGE_ONE 30000
#define STAGE_TWO 50000 // array size : 2000
#define STAGE_THREE 70000 // array size : 4000

static int stage_two = 1;
static int stage_three = 1;
static int stage_danger = 1;
...
static int sfq_set_request(struct request_queue *q, struct request *rq, struct bio *bio, gfp_t gfp_mask){
     ...
     sfqr = (struct sfq_request*)kmalloc(sizeof(struct sfq_request)*STAGE_ONE, gfp_mask);
...
static void sfq_add_request(struct request_queue *q, struct request *rq){
  ...
  //  }
    ...
    if(sfqd->size >= 20000) {
        ...
        sfqd->requests = (sfq_request **)krealloc(sfqd->requests, STAGE_TWO * sizeof(sfq_request *), GFP_KERNEL) ;
     } else if (sfqd->size > 40000){
        ...
        sfqd->requests = (sfq_request **)krealloc(sfqd->requests, STAGE_THREE * sizeof(sfq_request *), GFP_KERNEL) ;
    } else {
