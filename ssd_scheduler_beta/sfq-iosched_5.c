...
static int sfq_init_queue(struct request_queue *q, struct elevator_type *e){
  ...
  //NOOP
	...
	q->elevator = eq;
	spin_unlock_irq(q->queue_lock);
  printk("INIT_SFQ TESTING_DISPATCH_REQUEST\n");
...
static int sfq_dispatch(struct request_queue *q, int force){
  ...
  next_dispatch:
      ...
      if(sfqd->sfqr_size && list_empty(&sfqd->os_sfqq->queue)){
          while(list_empty(&sfqd->os_sfqq->queue)){
            sfqq = list_next_entry(sfqd->os_sfqq, queuelist);
            sfqd->os_sfqq = sfqq;
            if(sfqd->sfqq_total_seek == sfqd->sfqq_size){
              sfqd->sfqq_total_seek = 0;
      ...
      // how about using a while loop and get the ones that has requests?
  }

  printk("\t\tCHARA pid: %d sfqq_list_empty: %d sfqr->size: %d\n", sfqd->os_sfqq->pid, list_empty(&sfqd->os_sfqq->queue), sfqd->sfqr_size);

  // check for left requests
  // if(list_empty(&sfqq->queue)) goto dispatch_section;
  ...
  if(sfqr){
    ...
    // FRISK add request into heap_queue
