...
static int sfq_dispatch(struct request_queue *q, int force){
  ...
  // currently dispatching
      ...
      // if(!list_empty(&sfqq->queue)){
      //   printk("SFQQ->QUEUE IS NOT EMPTY PID: %d\n", sfqd->os_sfqq->pid);
      //
      ...
      //
      // } else {
      //   printk("\t\t\t\tSFQQ->QUEUE IS EMPTY PID: %d\n", sfqd->os_sfqq->pid);
      // }
  ...
  // sfqr = list_first_entry_or_null(&sfqq->queue, struct sfq_request, queuelist);
  // if(sfqr){
  //     printk("NEXT_DISPATCH START_TAG: %d\n", sfqr->start_tag);
  // }else goto dispatch_section;
