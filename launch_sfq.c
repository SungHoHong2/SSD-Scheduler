...
static int sfq_init_queue(struct request_queue *q, struct elevator_type *e){
  ...
  //  3656096,6471991 write /media/sf_SSD-Scheduler/test_results/2017_08_19_write/sfq_test_02.txt
...
static int sfq_set_request(struct request_queue *q, struct request *rq, struct bio *bio, gfp_t gfp_mask){
  ...
  // printk("------>before the set request starts\n");
	// if(!sfqd) return 0;
	// if(!rq) return 0;
	// if(!bio) return 0;

  // printk("------>after the set request starts\n");
...
static int sfq_dispatch(struct request_queue *q, int force){
  ...
  if(sfqd->heap_size>0){
      ...
      // chara
      printk("DISPATCH from HEAP_QUEUE pid: %d  heap_size: %d\n", sfqd->os_sfqq->pid, sfqd->heap_size);
